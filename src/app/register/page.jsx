...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status === 201) {
        setMsg({ type: 'success', text: 'Registered — redirecting to login…' });
        setTimeout(() => router.push('/login'), 700);
      } else {
        setMsg({ type: 'error', text: body.error || 'Registration failed' });
      }
    } catch (err) {
      console.error(err);
    ...
    <div style={{ padding: '32px 24px', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif', minHeight: '100vh', background: 'linear-gradient(135deg, #fef3c7 0%, #fde047 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <main style={{ maxWidth: 460, width: '100%', background: '#fff', padding: 40, borderRadius: 20, boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 18 }}>
          <label>
            <div style={{ fontSize: 14, marginBottom: 8, color: '#374151', fontWeight: 600 }}>Username</div>
            <input
              value={name}
          ...
          <label>
            <div style={{ fontSize: 14, marginBottom: 8, color: '#374151', fontWeight: 600 }}>Password</div>
            <input
              value={password}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8 }}>
            <button disabled={loading} style={{ padding: '14px 16px', borderRadius: 10, background: 'linear-gradient(135deg, #fde047 0%, #fbbf24 100%)', color: '#92400e', border: 'none', cursor: loading ? 'default' : 'pointer', fontWeight: 700, fontSize: 15, boxShadow: '0 4px 12px rgba(251, 191, 36, 0.4)', transition: 'all 0.2s' }} onMouseOver={e => { if (!loading) e.target.style.transform = 'translateY(-2px)' }} onMouseOut={e => e.target.style.transform = 'translateY(0)'}>
              {loading ? 'Creating…' : 'Register'}
            </button>
            <button type="button" onClick={() => router.push('/login')} style={{ padding: '14px 16px', borderRadius: 10, border: 'none', background: '#f3f4f6', color: '#6b7280', cursor: 'pointer', fontWeight: 600, fontSize: 15, transition: 'all 0.2s' }} onMouseOver={e => e.target.style.background = '#e5e7eb'} onMouseOut={e => e.target.style.background = '#f3f4f6'}>
              Back to Login
            </button>
          </div>
